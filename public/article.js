...
function renderArticle(article, software) {
  const root = document.getElementById("articleDetail");
  root.innerHTML = `<section class="article-cover">${article.coverUrl ? `<img src="${escapeAttr(article.coverUrl)}" alt="">` : ""}<div><p class="eyebrow">Article</p><h1>${escapeHtml(article.title)}</h1><p>${escapeHtml(article.summary || "")}</p></div></section><section class="article-body"><article>${renderContent(article.content)}</article>${renderRelated(software)}</section>`;
}

function renderRelated(software) {
  if (!software.length) return "";
  return `<aside class="article-related"><h2>相关软件</h2>${software.map(item => `<a href="/download.html"><strong>${escapeHtml(item.name)}</strong><span>${escapeHtml(item.description || "查看下载")}</span></a>`).join("")}</aside>`;
}
